...
# cautare Greedy
oras_plecare = 0
oras_destinatie = 12 # destinatia va fi mereu Bucuresti!
...
while not gasit and len(noduri) > 0:
    ...
    if nod == oras_destinatie:  # orasul curent este destinatia
        ...
        # Adaugam sortat dupa distanta in km pana la Bucuresti in noduri orasele care sunt conectate de nod
        for i in range(n):
            if a[i][nod] != 0 and viz[i] == 0:
		...
		noduri = np.append(noduri, i)  # adaugam nodul la sfarsit in lista de noduri
                # sortam crescator lista de noduri in functie de aproximarea 
		# distantei in linie dreapta pana la Bucuresti folosind programare functionala:
                noduri = [j[0] for j in sorted(zip(noduri, h[noduri]), key=lambda p: p[1])]
